...
const localDuplicatePhoneSet = computed(
  () => new Set(localDuplicatePhones.value),
);
const phoneFieldErrors = computed(() =>
  form.value.phones.map((item) => getPhoneFieldError(item.phone)),
);
const hasPhoneFieldError = computed(() =>
  phoneFieldErrors.value.some((item) => Boolean(item)),
);

const provinceOptions = chinaPcaCode;
const cityOptions = computed<ChinaPcaNode[]>(() => {
  const province = provinceOptions.find(
    (item) => item.code === form.value.province,
  );
...
const normalizePhonesForCheck = (
  phones: CustomerFormPhone[] = form.value.phones,
) => {
...
const syncBackendDuplicatePhones = () => {
  ...
  duplicatePhones.value = duplicatePhones.value.filter((phone) =>
    currentValidPhones.has(phone),
  );
}
...
const isDuplicatePhone = (phone: string) => {
  ...
  return Boolean(
    normalized &&
    (duplicatePhoneSet.value.has(normalized) ||
      localDuplicatePhoneSet.value.has(normalized)),
  );
}
...
watch(
  ...
  ([open]) => {
    ...
    form.value = {
      ...
      nextTime: props.customer?.nextTime ?? "",
...
const handleSubmit = async () => {
  ...
  const payload: CustomerFormPayload = {
    ...
    nextTime: form.value.nextTime || undefined,
  ...
  <Dialog :open="open" @update:open="(val) => emit('update:open', val)">
    ...
    >
          ...
          <div class="grid gap-4 sm:grid-cols-2">
            <div class="space-y-2 sm:col-span-2">
              <Label for="customer-name"
                ><span class="mr-1 text-destructive">*</span>客户名称</Label
              >
              <Input
                id="customer-name"
                v-model="form.name"
            ...
            <div class="space-y-2 sm:col-span-2">
              <div class="flex items-center justify-between">
                <Label
                  ><span class="mr-1 text-destructive">*</span>联系电话</Label
                >
                <Button
                  type="button"
                  variant="outline"
                ...
                >
                  <div class="flex gap-2 items-center">
                    <Input
                      ...
                      class="flex-1"
                      :class="
                        isDuplicatePhone(phone.phone) || phoneFieldErrors[idx]
                          ? 'border-destructive focus-visible:ring-destructive'
                          : ''
                      "
                      @update:model-value="
                        (value) => handlePhoneInput(idx, value)
                      "
              ...
              <p
                v-if="localDuplicatePhones.length > 0"
                class="text-xs text-destructive"
              >
                当前表单手机号重复：{{ localDuplicatePhones.join("、") }}
              </p>
              <p
                v-if="duplicatePhones.length > 0"
                class="text-xs text-destructive"
              >
      ...
      <DialogFooter class="shrink-0 border-t px-6 py-4">
        ...
        <Button
          type="button"
          :disabled="submitting || checkingUnique || hasPhoneFieldError"
          @click="handleSubmit"
        >
