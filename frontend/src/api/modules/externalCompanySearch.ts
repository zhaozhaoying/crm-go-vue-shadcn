...
export function createExternalCompanySearchTasks(data: CreateExternalCompanySearchTasksRequest) {
...
export function listExternalCompanySearchTasks(params: ListExternalCompanySearchTasksParams = {}) {
...
export function listExternalCompanySearchResults(taskId: number, params: ListExternalCompanySearchResultsParams = {}) {
...
export function listAllExternalCompanySearchResults(params: ListExternalCompanySearchResultsParams = {}) {
  return request<ExternalCompanySearchResultListResult>({
    method: "GET",
...
export function listExternalCompanySearchEvents(taskId: number, afterSeq = 0, limit = 100) {
...
export function connectExternalCompanySearchStream(options: ExternalCompanySearchStreamOptions) {
  const apiBase = ((import.meta.env.VITE_API_BASE_URL as string | undefined)?.trim()) ||
    ((typeof window !== "undefined" && (window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1"))
      ? "http://localhost:8080/api"
      : `${window.location.origin}/api`)
  const wsBase = apiBase.replace(/^http/i, "ws")
  const url = new URL(`${wsBase}/v1/external-company-search/tasks/${options.taskId}/stream`)
  ...
  socket.onmessage = (messageEvent) => {
    try {
      const parsed = JSON.parse(String(messageEvent.data)) as ExternalCompanySearchEvent
