...
func (h *TelemarketingRecordingHandler) GetDetail(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, service.ErrMiHuaTelemarketingRecordingConfigNeeded):
			Error(c, http.StatusBadRequest, 91236, "缺少米话电销录音配置，请先在系统设置中配置 MIHUA_CALL_RECORD_TOKEN，并确认根目录 .env 中已配置 MIHUA_TELEMARKETING_RECORDING_LIST_URL 和 MIHUA_CALL_RECORD_SOURCE_ORIGIN")
...
func (h *TelemarketingRecordingHandler) Sync(c *gin.Context) {
	...
	if err != nil {
		switch {
		case errors.Is(err, service.ErrMiHuaTelemarketingRecordingConfigNeeded):
			Error(c, http.StatusBadRequest, 91241, "缺少米话电销录音配置，请先在系统设置中配置 MIHUA_CALL_RECORD_TOKEN，并确认根目录 .env 中已配置 MIHUA_TELEMARKETING_RECORDING_LIST_URL 和 MIHUA_CALL_RECORD_SOURCE_ORIGIN")
		case errors.Is(err, service.ErrMiHuaTelemarketingRecordingRequestFail):
