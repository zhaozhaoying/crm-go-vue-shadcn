...
```bash
cd "/www/wwwroot/crm-go-shadcn/mihua-token-fetcher" && .venv/bin/mihua-token-fetcher --env-file "/www/wwwroot/crm-go-shadcn/.env" --headless --write-db
```

如果你还想保留 `.env` 里的副本：

```bash
cd "/www/wwwroot/crm-go-shadcn/mihua-token-fetcher" && .venv/bin/mihua-token-fetcher --env-file "/www/wwwroot/crm-go-shadcn/.env" --headless --write-db --write-env
```
